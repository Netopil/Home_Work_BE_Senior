...
def get_file_names(path, extensions="all"):
    ...
    Overall description: takes file path and extensions and return file names in the specific path according the
    extensions. If second parameter is empty, all extensions are chosen"""
    ...
    try:
        ...
        # get only files with required extensions
            ...
            for file in folder_list:
                filename, file_extension = os.path.splitext(file)
                splitted_extensions = extensions.split(",")
                stripped_extensions = [i.strip(' ') for i in splitted_extensions]
                if file_extension and (file_extension[1:] in stripped_extensions):
                    file_list.append(filename + file_extension)
        print("Path with files:", path.title())
        print("File extensions:", extensions.title())
        if not file_list:
            print("Files with extensions", extensions, " haven't been found")
        else:
            print("Valid file list: ",file_list)
    return file_list
...
def get_valid_images(path, file_list):
    ...
    Overall description: takes file list and check if the files are valid images than return list of images. """
...
def load_send_files():
    """Establish connection, receive message: path, call functions, send message: path and image name"""
